...
Strava has restricted API access to subscription accounts only. Without a
Strava subscription this app can no longer authenticate or read activities
at all, so it is no longer usable. Development has stopped and the hosted
infrastructure (backend VM, DNS, API keys) has been torn down.
====
